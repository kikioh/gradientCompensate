% Phase-sensitive HSQC pulse sequence. Apply spin locking for parallel NMR case. Syntax:
%
%    fid=hsqc_grad_EA_SL(spin_system,parameters,H,R,K,Hz,grad_self,grad_coup,pulse)
%
% Parameters:
%
...
% Outputs:
%
%     fid.pos,fid.neg -  two components of the States quadrature signal
%
% Note: natural abundance experiments should make use of the isotope dilution functionality. 
%       See dilute.m function.
%
% ref: 1992_JMR_gradient-Enhanced Heteronuclear Correlation Spectroscopy. 
%      Theory and Experimental Aspects
% ref: <http://spindynamics.org/wiki/index.php?title=hsqc.m>
%
% Mengjia He, 2024.03.28
%
% ref: <http://spindynamics.org/wiki/index.php?title=hsqc.m>

function fid=hsqc_grad_EA_SL(spin_system,parameters,H,R,K,Hz,grad_self,grad_couple,pulse_dt,SLpulse)
...
% assemble the spin locking pulse
SL = cell(1,2);
SL{1} = [{zeros(size(pulse_dt)),zeros(size(pulse_dt))},SLpulse.C13];
SL{2} = [SLpulse.H1,{zeros(size(pulse_dt)),zeros(size(pulse_dt))}];
...
switch parameters.chnIndex
    case 1
        ...
        rho_stack=shaped_pulse_xy(spin_system,L,{Hz},grad_self(1),pulse_dt,rho_stack);

        % spin locking during first gradient pulse
        rho_stack=shaped_pulse_xy(spin_system,L,{Lx_F2,Ly_F2,Lx_F1,Ly_F1,Hz},[SL{1},grad_couple(1)],pulse_dt,rho_stack);
        ...
        rho_stack_pos=shaped_pulse_xy(spin_system,L,{Hz},grad_self(2),pulse_dt,rho_stack);
        rho_stack_neg=shaped_pulse_xy(spin_system,L,{Hz},{-1*grad_self{2}},pulse_dt,rho_stack);

        % spin locking during second gradient pulse
        rho_stack_pos=shaped_pulse_xy(spin_system,L,{Lx_F2,Ly_F2,Lx_F1,Ly_F1,Hz},[SL{2},grad_couple(2)],pulse_dt,rho_stack_pos);
		rho_stack_neg=shaped_pulse_xy(spin_system,L,{Lx_F2,Ly_F2,Lx_F1,Ly_F1,Hz},[SL{2},{-1*grad_couple{2}}],pulse_dt,rho_stack_neg);
    ...
    case 2
        ...
        % spin locking during second gradient pulse
        rho_stack_pos=shaped_pulse_xy(spin_system,L,{Lx_F2,Ly_F2,Lx_F1,Ly_F1,Hz},[SL{2},grad_couple(2)],pulse_dt,rho_stack);
		rho_stack_neg=shaped_pulse_xy(spin_system,L,{Lx_F2,Ly_F2,Lx_F1,Ly_F1,Hz},[SL{2},{-1*grad_couple{2}}],pulse_dt,rho_stack);

       % second gradient pulse
        rho_stack_pos=shaped_pulse_xy(spin_system,L,{Hz},grad_self(2),pulse_dt,rho_stack_pos);
        rho_stack_neg=shaped_pulse_xy(spin_system,L,{Hz},{-1*grad_self{2}},pulse_dt,rho_stack_neg);
end
