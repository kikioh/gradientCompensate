...
data = load('SL-sin-13C-1ms-RF4kHzdev17-BW6kHz-05G-homoJcompen.mat');
...
%% Spinach parameters
sys.magnet = 11.74;                                  
sys.isotopes = {para.nuclei,para.nuclei};
...
rho_init=state(spin_system,'Lx',2);
rho_init=rho_init/norm(full(rho_init),'fro');

% Get the control operators
Lx=operator(spin_system,'Lx','all'); 
Ly=operator(spin_system,'Ly','all'); 
Lz = operator(spin_system,'Lz',2);

% Calculate the Hamiltonian
Hz = hamiltonian(assume(spin_system,'labframe','zeeman'))/sys.magnet; 
Hc = hamiltonian(assume(spin_system,'nmr','coupling')); 

%% fidelity with v0 and v1 offsets
unit_opt = eye(size(Hz,1));
eta = zeros(numBW,numRF); Bg_temp = Bg{11};    
for k = 1:numBW                             % v0 offset
    ...
    parfor s = 1:numRF                      % v1 offset
        ...
        % compensated
        rho = shaped_pulse_xy(spin_system,H,{Lx,Ly,Hz},...
            {Cx{1}*coeff,Cy{1}*coeff,Bg_temp},pulse_dt,rho_init);
        
        eta(k,s) = rho_init' * rho;
...
xticks([-para.spin.BW/2,0,para.spin.BW/2]/1e3);
yticks([0.85,1,1.15]*para.RF.A/1e3);
